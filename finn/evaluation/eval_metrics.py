...
def train_zy_head(args, experiment, trunk, discs, train_data, val_data):
    ...
    disc_y_from_zy_copy = tabular_model(
        discs.args, input_dim=discs.wh * args.zy_dim, depth=args.inv_disc_depth, batch_norm=False
    )
    ...
    head_optimizer = Adam(
        disc_y_from_zy_copy.parameters(), lr=args.lr, weight_decay=args.weight_decay
    )
            ...
            with torch.no_grad():
                ...
                for i, (x, s, y) in enumerate(val_loader):
                    ...
                    if args.dataset == 'adult':
                        ...
                        acc = torch.sum(
                            F.softmax(zy_new[:, : args.y_dim], dim=1).argmax(dim=1) == y
                        ).item()
                    ...
                    if (
                        args.dataset == 'cmnist'
                        and epoch % (int(args.clf_epochs / 4)) == 0
                        and i == 0
                    ):
                        ...
                        log_images(
                            experiment, recon_all, f'reconstruction_all_{epoch}', prefix="eval"
                        )
                        ...
                        log_images(
                            experiment, recon_ys, f'reconstruction_ys_{epoch}', prefix="eval"
                        )
                        log_images(
                            experiment, recon_yn, f'reconstruction_yn_{epoch}', prefix="eval"
                        )
                        log_images(
                            experiment, recon_sn, f'reconstruction_sn_{epoch}', prefix="eval"
                        )
